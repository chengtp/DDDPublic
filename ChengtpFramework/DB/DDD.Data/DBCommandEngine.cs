...
namespace DDD.Data
{
    ...
    {
        const string identityKey = "DDD_data_defaultdbcommandengine";
        ...
        {
            if (command == null || DataManager.GetDBServer == null)
            {
                return null;
            }
            var servers = DataManager.GetDBServer(command);
            if (servers.IsNullOrEmpty())
            {
                throw new DDDException("database server information not obtained from ICommand");
            }
        ...
        void VerifyServerEngine(params ServerType[] serverTypes)
        {
            ...
            {
                if (!DataManager.DbEngines.TryGetValue(serverType, out var dbEngine) || dbEngine == null)
                {
                    throw new DDDException($"no execution engine for configuring database type {serverType}");
                }
            }
        }
